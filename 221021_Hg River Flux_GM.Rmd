...
ggplot(AllBAR3, aes(x=Month, colour=Station.name)) +
  geom_line( aes(y=DisMean_m3s/300), size=0.8) +
  geom_boxplot( aes(y=THg, group=Month))+
  geom_jitter( aes(y=THg), width=0.15)+
  facet_wrap(type~., scales="free")

#make average of flow
AllBARy <- na.omit(AllBAR3) %>% 
  filter(!Year == "2021") %>% 
  group_by(Month, Station.name)%>%
  summarise(mean_Dis=mean(DisMean_m3s))

AllBARx <- na.omit(AllBAR3) %>% 
  filter(!Year == "2021") %>% 
  group_by(Month, Station.name)%>%
  summarise(mean_THg=mean(THg))

AllBAR6 <- merge(AllBARy, AllBARx, by=c("Station.name","Month"))

AllBAR6$type <- as.factor(ifelse(AllBAR6$Station.name == "Northern Dvina", "4",
                          ifelse(AllBAR6$Station.name == "Malselv", "1", 
                          ifelse(AllBAR6$Station.name == "Altaelva", "1", 
                          ifelse(AllBAR6$Station.name == "Tanaelva", "2",
                          ifelse(AllBAR6$Station.name == "Pasvikelva", "2",
                          ifelse(AllBAR6$Station.name == "Tuloma", "2", "")))))))


#Try to make plot less messy without the box
ggplot(AllBAR6, aes( x=Month, group=Station.name)) +
  geom_line(aes(y=mean_Dis/300, colour=Station.name))+
  geom_point(aes(y=mean_THg, colour=Station.name))+
  facet_wrap(type~., scales="free")
  
  geom_boxplot(aes(y=mean_THg, group=Month, colour=Station.name))+

  'geom_point(aes(y=THg))+  
